...
export default function CompanyProfilePage() {
  ...
  const handleFileSelect = (e) => {
    ...
    if (file.size > 3 * 1024 * 1024) {
      toast.error("Ukuran file logo maksimal 3MB. Silakan kompres gambar terlebih dahulu.");
      return;
    }

    const validTypes = ["image/png", "image/jpeg", "image/jpg", "image/webp", "image/svg+xml"];
    if (!validTypes.includes(file.type)) {
      toast.error("Format file harus berupa PNG, JPG, JPEG, SVG, atau WEBP.");
      return;
    }
              ...
              <div className="space-y-2 pt-1">
                ...
                {selectedFile && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 flex items-center justify-between">
                    <span className="text-xs text-amber-900 font-medium truncate max-w-[170px]">
                    ...
                    <Button
                      ...
                      className="bg-navy text-white text-xs h-7 px-2.5 font-bold"
                    >
                      {uploadingLogo ? <Loader2 size={12} className="animate-spin" /> : "Upload"}
                    </Button>
                  </div>
                )}

                <p className="text-[11px] text-slate-400 leading-tight">
                  Format didukung: PNG, JPG, JPEG, SVG, WEBP. Disimpan aman pada Cloud Storage.
                </p>
              </div>
            )}
          </div>
