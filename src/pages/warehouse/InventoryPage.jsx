...
export default function InventoryPage() {
  ...
  const submitHandover = async () => {
    ...
    setBusy(true);
    try {
      await api.post("/stock/handovers", {
        ...handoverForm,
        items: validItems,
      });
      toast.success(handoverForm.auto_confirm ? "Serah terima stok berhasil dikonfirmasi dan stok berpindah ke Sales!" : "Draft serah terima stok berhasil dibuat.");
  ...
  const exportStockToCsv = () => {
    ...
    link.setAttribute("download", `Laporan_Stok_Inventaris_${filterDate}.csv`);
  ...
  return (
    <div className="space-y-6" data-testid="inventory-page">
      ...
      {activeTab === "handovers" && (
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <Button onClick={handleOpenNewHandover} className="bg-navy text-white text-xs font-bold gap-1 h-9">
              <Plus size={14} /> Buat Serah Terima Baru
            </Button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2 pt-6">
                ...
                <label htmlFor="auto_post_rcv" className="text-xs font-medium text-slate-700 cursor-pointer">
                  Langsung posting ke stok Gudang (Auto-Post & Update Inventaris)
                </label>
              </div>
            </div>
          </div>
      ...
      {/* DIALOG: BUAT SERAH TERIMA PAGI (HANDOVER) */}
      <Dialog open={handoverDialogOpen} onOpenChange={setHandoverDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="font-heading text-lg font-bold text-navy flex items-center gap-2">
              <ArrowRightLeft size={18} /> Serah Terima Stok Pagi (Warehouse → Sales)
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500">
              Pilih salesman dan tentukan kuantitas stok yang diserahkan dari gudang sebelum berangkat ke lapangan.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-bold">Tanggal Operasional *</Label>
                <Input
                  type="date"
              ...
              <div className="border border-slate-200 rounded-lg overflow-hidden max-h-60 overflow-y-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50">
                      <TableHead className="text-xs">Produk SKU</TableHead>
                      <TableHead className="text-xs text-right">Stok Gudang Tersedia</TableHead>
                      <TableHead className="text-xs text-right w-32">Kuantitas Serah (Qty)</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {handoverForm.items.map((it) => {
                      const whInv = inventoryItems.find((i) => (i.location_type === "WAREHOUSE" || !i.location_type) && i.sku_id === it.sku_id);
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="Contoh: Stok reguler rute Cianjur Kota"
          ...
          <DialogFooter className="gap-2">
            ...
            <Button disabled={busy} onClick={submitHandover} className="bg-navy text-white text-xs font-bold">
              {busy ? <Loader2 className="animate-spin mr-1" size={14} /> : <CheckCircle2 className="mr-1" size={14} />}
              Konfirmasi & Serahkan Stok
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                ...
                <Table>
                  ...
                  <TableBody>
                    {(selectedDetail.data.items || []).map((it, idx) => (
                      <TableRow key={idx}>
                        <TableCell>
                          <div className="font-bold text-navy">{it.sku_name || it.sku_code || it.sku_id}</div>
                          <div className="text-[10px] text-slate-400 font-mono">{it.sku_code}</div>
                        </TableCell>
                        {selectedDetail.type === "RECEIVING" && (
