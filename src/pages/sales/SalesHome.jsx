...
export default function SalesHome() {
  ...
  const doAttendance = async (type) => {
    setBusy(true);
    try {
      const pos = await getPosition();
      if (type === "in") {
        const r = await postQueued("/attendance/check-in", pos);
        ...
        const { data } = await api.post("/attendance/check-out", pos);
      ...
      <div className="bg-gradient-to-br from-navy via-navy to-navy-dark rounded-2xl p-5 text-white shadow-lg space-y-4 border border-navy-light/20 relative overflow-hidden">
        ...
        {!onDuty && att?.status !== "OFF_DUTY" && (
          ...
          >
            ...
            Absen Masuk (Validasi GPS Kantor & Jam Shift)
          </Button>
        )}
